        ...
        while True:
            ...
            if SQL_flag:
                ...
                if check(proxy, handle):
                    sql = "insert into proxy(proto,ip,port,country,anonymity,type,status,source) value ('%s','%s','%s','%s','%s','%s','%s','%s')" % (
                        proxy.get("proto"), proxy.get("ip"), proxy.get("port"), proxy.get("country"), proxy.get("anonymity"),
                        proxy.get("type"), "1", "https://github.com/stamparm/fetch-some-proxies")
                    mysql.insert(sql)

    except Exception as e:
        pass
        # logging.exception("aaaa")


def worker1(queue, handle=None):
    """
    从队列里面随机获取代理地址，访问 可以获取原地址ip的 url，如果返回的ip和代理ip相同，则证明改代理可用
    :param queue:
    :param handle:
    :return:
    """
    ...
    try:
        while True:
            ...
            sys.stdout.flush()
            start = time.time()
            proxy["proto"] = proxy["proto"].replace("https", "http")
            if SQL_flag:
                sql = "select id from proxy where proto='{}' and  ip='{}' and  port='{}'".format(
                    proxy.get("proto"), proxy.get("ip"), proxy.get("port"))
                if mysql.select(sql):
                    print("重复出现{}://{}:{}".format(proxy.get("proto"), proxy.get("ip"), proxy.get("port")))
                    continue
            ...
            if (result or "").strip() == proxy["ip"].encode("utf8"):
                ...
                if latency < timeout:
                    if SQL_flag:
                        sql = "insert into proxy(proto,ip,port,country,anonymity,type,status,source) value ('%s','%s','%s','%s','%s','%s','%s','%s')" % (
                        proxy["proto"], proxy["ip"], proxy["port"], proxy["country"], proxy["anonymity"], proxy["type"],
                        "1", "https://github.com/stamparm/fetch-some-proxies")
                        mysql.insert(sql)
...
def make_parse():
    ...
    parser.add_option("--max-latency", dest="maxLatency", type=float,
                      help="Maximum (tolerable) latency in seconds (default %d)" % TIMEOUT)
    parser.add_option("--no-https", dest="noHttps", action="store_true",
...
def main():
    ...
    process = subprocess.Popen("curl -m %d -A \"%s\" %s" % (TIMEOUT, USER_AGENT, random_ifconfig()), shell=True,
    ...
    if not all(_ in content for _ in ("proto", "country", "anonymity")):
        exit(
            "[!] something went wrong during the proxy list retrieval/parsing ('%s'). Please check your network settings and try again" % content[
                                                                                                                                          :100])
    ...
    if any((options.country, options.anonymity, options.type, options.port)):
        ...
        for proxy in proxies:
            ...
            if options.port and not proxy["port"] in options.port:
                continue
            if options.anonymity and not re.search(options.anonymity, "%s (%s)" % (
                    proxy["anonymity"], ANONIMITY_LEVELS.get(proxy["anonymity"].lower(), "")), re.I):
                continue
    ...
    sys.stdout.write("[i] testing %d proxies (%d threads)...\n\n" % (
        len(proxies) if not FALLBACK_METHOD else sum(proxy["proto"] in ("http", "https") for proxy in proxies),
